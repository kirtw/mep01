...
---------------
local function termMch(inputArg,Si)
    inC = inputArg:sub(Si,Si)
...
local function getTokR1(item,grammar)
    local gsi = item.rule
...
local function complete(S, Si, Sj, grammar)
   ...
   for old_item_index, old_item in ipairs(S[item.start]) do
      io.write("\n item done, Complete loop-",Si,"/",Sj,"  old:",old_item_index,"<>",type(old_item))
      local TokR1= getTokR1(grammar, old_item) 
 --     tokVal= symbol(grammar, old_item)  io.write("\n tokVal:",tokVal)
      local ngri= name(grammar, item)
      io.write("\n Test getTokR1:",type(tokr1)," ?== ",ngri," : name(gram... ")
      TokR1 = getTokR1(grammar,old_item)
...
-- update terminal match item?              
   local itemSV = S[Si][Sj]
     itemSV.next = item.next + 1
   SSC(S,Si,Sj,Si+1,itemSV,"SCAN")
--      append(S[Si+1], { rule  = item.rule,
--                              next  = item.next + 1,
--                              start = item.start },"SCAN")
...
--   
   ...
   for gsi = 1, #Bgram do
      ...
      if Bgram[gsi].name == Bgram.start_rule_name then
           Sj = Sj + 1
           local testItem = S[1][1]  
           io.write("Type of testItem",type(testItem),"\n")
           local itemSV = { rule  = gsi, start = 1, next  = 1 }
--              -- sic vs   SSC(S,1,Sj,1,itemSV,"Init")
--             S[1][1] = itemSV
--             S[1] = Sj  
           io.write("Init:",type(S)," Sj=",Sj,"\n")             
      end  -- end if-then
   end  -- end for/do
   ...
   while Si <= #S do
      local Sj = 1
      while Sj <= #S[Si] do
         local symbol = getTokR1(Bgram, S[Si][Sj])
...
local function print_Sec(S, Si, grammar)
--   for i, set in ipairs(S) do
     local set = S[Si]
      io.write('    === ', Si-1, ' ===\n')
      pp = pretty_printer()
