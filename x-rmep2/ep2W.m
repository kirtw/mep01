ep2W  ;CKW/ESC i31oct22 rMP1/ ;20230111-50;Write sr for parser data structures
;
;
;
;*
...
Wmdk   NEW Q S Q=""  D ^devIB  ; : PB
       ...
       W !!,"--",!," Grammar AND then SCF State Tables Dump",!!
       D WSC
       D WSCLua
       ;D WGR      Write Grammar GRk(), Gxi() to the file
       W !!,"--",!
       D CFM^devIO(devm)
       Goto Q
;* Common Exit
...
WS(Si,M)  W:$X ! W !,"  SEarly:",Si-1,"   ====++Si ",Si," ++====  "
       ...
       F Sj=1:1 Q:$D(SCF(Si,Sj))=0   DO  ;
         .D GFL^jfm("svSSq,ruid,ruab,ruLst,dot,IPs,IPe,tokR1,tokTy,frm",itemFL) ;
         .D Witem
       W !
       Q
