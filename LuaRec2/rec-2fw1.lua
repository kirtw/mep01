...
---------------
--  new Tmch and tokR1 replaces var symbol (various types/str,func,nil)
--      and  next_symbol
--
local function Tmch(input,i,Ttok)
    local CL = symCL[Ttok]['CL']
    if CL==nil then io.write("Err mch C/i \n") end
    local mch = false
    local C = input.byte(i)
    for ci = 1, CL.len() do if CL.byte(ci) == C then mch = true end end
    io.write("Term Mch: ",Ttok,"-",mch,"  '",C,"'  ci=",ci,"\n")
    return mch
end
-- next element in the rule of this item
local function tokR1(grammar, item)
    local ruid = item.rule
    local ruLst = grammar[ruid]
    local ruLen = #ruLst    
    local dot = item.next
    if dot > ruLen then return "C" end  -- Complete
    local tokR1 = ruLst[dot+1]
    local tok2 = grammar[ruid][dot+1]
    if tokR1 ~= tok2 then 
            io.write("Err tokR1 v tok2  ",tokR1,"~=",tok2,"  #",ruid," \n")
            end
    if tokR1 == nil then 
         io.write("Err tokR1 nil gr#",ruid,"  len:",ruLen,"  dot:",dot,"\n")
         tokR1="X"
         end
   return tokR1
end
...
-- compares two items for equality (needed for safe SavUniqItem)
...
function SaveItem(set, item)
   set[#set+1] = item
end

-- Adds an item at the end of the Earley set, **unless already present**
function SavUniqItem(set, item)
   for i = 1, #set do
      if equal(item, set[i]) then
         return
      end
   end
   SaveItem(set, item) -- the item wasn't already there, we add it
end
...
local function COMP(S, i, j, grammar)
   ...
   for old_item_index, old_item in ipairs(S[item.start]) do
      local runa = name(grammar, item)
      local tokR1 = tokR1(grammar, old_item)
      if tokR1 == runa then
         SavUniqItem(S[i], { rule  = old_item.rule,
...
local function SCAN(S, i, j, Ttok, input)
   local item = S[i][j]
      if Tmch(input, i, Ttok) then -- terminal symbols are predicates
        if S[i+1] == nil then S[i+1]  = {} end
        SaveItem(S[i+1], { rule  = item.rule,
                              next  = item.next + 1,
                              start = item.start })
       end
end
...
local function PRED(S, i, symbol, grammar)
   for rule_index, rule in ipairs(grammar) do
      if rule.name == symbol then
         SavUniqItem(S[i], { rule  = rule_index,
                        next  = 1 ,
                        start = i })
      end
   end
...
local function BUILD(grammar, input)
   ...
   while i <= #S do
      ...
      while j <= #S[i] do
         local item = S[i][j]
         if type(item) == "nil" then io.write("Err item B loop",i,".",j,"\n") end
         local ruid = item.rule
         local tokR1 = tokR1(grammar,item)
           if tokR1 == nil then io.write("Err tokR1 nil",i,".",j,"  ","\n") end
--       --  local ty2 = symTy[tokR1]
         local ty = "R" 
         if ruid >= 9 then ty = "T" end
         local ty3 = grammar[ruid].ty
         if ty ~= ty3 then io.write("Err ty vs ty3 ?",ty," ~= ",ty2,"ruid:",ruid,"\n") end
         io.write("Bloop ",i,".",j,"  ty:",ty," ruid:",ruid," tokR1:",tokR1,"\n")         
         if     tokR1 == "C"      then COMP(S, i, j, grammar)
         elseif ty == "T" then SCAN    (S, i, j, tokR1, input)
         elseif ty == "R" then PRED (S, i, tokR1, grammar)
...
local Grammar = {
   start_rule_name = 'Sum',
   { name = 'Sum',ty="R" , 'Sum'      , TPsum, 'Product'},
   { name = 'Sum',ty="R"    , 'Product'  ,                       },
   { name = 'Product',ty="R", 'Product'  , TPfac, 'Factor' },
   { name = 'Product',ty="R", 'Factor'   ,                       },
   { name = 'Factor',ty="R" , TCop  , 'Sum', TCcp      },
   { name = 'Factor',ty="R" , 'Number'                           },
   { name = 'Number',ty="R" , Tdig, 'Number'              },
   { name = 'Number',ty="R" , Tdig,                       },
   { name = 'Tdig',ty="T",CL='0123456789' },
   { name = 'TCop',ty="T",CL='(' },
   { name = 'TCcp',ty="T",CL=')' },
   { name = 'TPsum',ty="T",CL='+-' },
   { name = 'TPfac',ty="T",CL='*/' },    
...
symTy ={}
symCL ={}
for ruid,ty in ipairs(Grammar) do 
      local name = Grammar[ruid].name
      local ty = Grammar[ruid].ty
      local CL = Grammar[ruid].CL
      symTy[name] = ty
      symCL[name] = CL
end
