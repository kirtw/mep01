dv2(dvM,VVL)  ;CKW/ESC i28Aug14 gmsa/ rddv3/ ; 20220515  ;  Debug Var lists 
; copy of ^dv avoid dependencies in rzro3./ for startup nearly bare,derive $zro
  ;
  ;     gboo, rz/     ^f    with ^fd*	TP Doc:  H7r/  aUtil/  dv/ 
  ;	or d ka^dv  Summarize and Kill Arrays
  ;   Refs ^fd*  in gboo/ rH/
  ;
  ;
  ...
  S %zshc=2
  NEW %KMO,%c
d2  I $G(dvM)="" S dvM="Err NOS ^dv" S:$G(VVL)="" VVL="id"
...
Ilog  S devdvlog="dvlog.txt" OPEN devdvlog:(newversion)
   U devdvlog W !,"Default ^dv log file ???  ",$ZD($H),!! D ^dvstk W !!!
   Q
;*
  ;Entry  b^dv  Save $I, USE 0 zsh ? Break
b(dvM,VVL) 
b2  NEW dBdv,%zshc,%3,%D,%KMO,%S,%i,%n,%v,%vn,%vv
    S dBdv=$IO USE 0 S %zshc=3 D d2 ;D ^dv($G(dvM),$G(VVL))
    B
    U dBdv
    Q
...
ka  S %KMO=1 G aa	;List One line and Kill Arrays     D ka^dv
la  S %KMO=0		;Just list			   D la^dv
aa  W:$X ! W "** Arrays-",! 
